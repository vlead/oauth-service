...
* Introduction
  ...
  I have also made a small script which is tangled out and can be run to do the whole setup
...
** Add the keys and secret to deployerConfiguration.xml
   Link to the directory [[../vlead-cas-overlay/vlead-cas-server/src/main/resources][here]] Open the deployerConfiguration.xml and
   add the Keys and secret of diiferent Identity Providers. Also set
   the Callback Url to /cas/login
* Pull the Repo and run the server 
...
* Key note
  All the important configurations are stored in
  src/vlead-cas-overlay/vlead-cas-server/src/main/resources Hence in
  the code make changes to this file followed by compiling the
