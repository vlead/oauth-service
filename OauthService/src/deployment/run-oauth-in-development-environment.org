#+TITLE: Running Oauth Service on Developement Environment
#+Author: VLEAD
#+Date: [2017-05-06 Fri]
...
* Creation of database
  ...
  - Show all databases
    ...
    show databases;
    #+END_EXAMPLE
  - If <oauthService> database is not present, create it
    #+BEGIN_EXAMPLE
    create database ouathService;
    #+END_EXAMPLE
...
* Initial make
  Run make from =oauth-service= directory to build the
...
* Creation of Virtual Environment
  ...
  + Create Virtual Environemnt in the =oauth-service= directory
  #+BEGIN_EXAMPLE
  virtualenv venv
  #+END_EXAMPLE
...
* Setting up dependencies
  Install all the dependencies for running the =oauthService= applciation.  The
  =requirements.txt= file is in the =build/code/deployment/= directory
  #+BEGIN_EXAMPLE
...
* Set SQLALCHEMY_DATABASE_URI
  ...
  use the database.
  #+BEGIN_EXAMPLE
  SQLALCHEMY_DATABASE_URI = 'mysql+mysqldbs://root:<yourpswd>@localhost/oauthService'
  #+END_EXAMPLE
...
* Access the application
  ...
  The application is accessed at =https://localhost:5000=
  

* API Documentation
  URL :
  [https://github.com/vlead/oauth-service/blob/master/src/runtime/rest/api.org] 
